...
def run_us_market_part() -> dict[str, dict[str, Any]]:
    """US MARKET PART (미국+유럽 지수) — 평일 오전 6:30 KST 실행. 지수 데이터 반환."""
    try:
        now_str = (datetime.now() + timedelta(hours=9)).strftime("%Y-%m-%d %H:%M")
        report = f"📊 <b>US MARKET PART ({now_str} KST)</b>\n━━━━━━━━━━━━━━━━━━\n\n"
...
def _fetch_summary_extra_news() -> str:
    """Bloomberg/Reuters RSS — Summary 전용 보충 소스."""
    cutoff_ts = mktime((datetime.now(timezone.utc) - HOURS_24).timetuple())
    lines = []
...
def _collect_all_news() -> list[dict[str, Any]]:
    ...
    for source_name, rss_url in RSS_FEEDS:
        for item in _fetch_rss(rss_url, source_name):
            ...
            if pt is not None and pt < cutoff_ts:
                continue
            combined.append(_normalize_item(item))

    logger.info("전체 뉴스 수집: %d건 (24h 필터 후)", len(combined))
    return combined
